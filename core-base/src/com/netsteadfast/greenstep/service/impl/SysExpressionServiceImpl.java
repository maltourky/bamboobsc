...
public class SysExpressionServiceImpl extends BaseService<SysExpressionVO, TbSysExpression, String> implements ISysExpressionService<SysExpressionVO, TbSysExpression, String> {
	...
	private String getQueryGridHql(String type, Map<String, Object> params) throws Exception {
		...
		if (Constants.QUERY_TYPE_OF_COUNT.equals(type)) {
			...
			hqlSb.append("	new com.netsteadfast.greenstep.vo.SysExpressionVO(m.oid, m.exprId, m.type, m.name, m.description) ");
		}
		hqlSb.append("FROM TbSysExpression m WHERE 1=1 ");
		if (params.get("exprId")!=null) {
			hqlSb.append("  and m.exprId = :exprId ");
		}		
		if (params.get("name")!=null) {
			hqlSb.append("  and m.name LIKE :name ");
		}
		if (params.get("type")!=null) {
			hqlSb.append("  and m.type = :type ");
		}			
