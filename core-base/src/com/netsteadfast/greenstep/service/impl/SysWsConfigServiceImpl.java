...
public class SysWsConfigServiceImpl extends BaseService<SysWsConfigVO, TbSysWsConfig, String> implements ISysWsConfigService<SysWsConfigVO, TbSysWsConfig, String> {
	...
	private Map<String, Object> getQueryGridParameter(SearchValue searchValue) throws Exception {
		...
		String wsId = searchValue.getParameter().get("wsId");
	...
	private String getQueryGridHql(String type, Map<String, Object> params) throws Exception {
		...
		if (Constants.QUERY_TYPE_OF_COUNT.equals(type)) {
			...
			hqlSb.append("	new com.netsteadfast.greenstep.vo.SysWsConfigVO(m.oid, m.wsId, m.system, m.type, m.beanId, m.publishAddress) ");
		}
		hqlSb.append("FROM TbSysWsConfig m WHERE 1=1 ");
		if (params.get("wsId")!=null) {
			hqlSb.append("  and m.wsId = :wsId ");
		}		
		...
		if (Constants.QUERY_TYPE_OF_SELECT.equals(type)) {
			hqlSb.append("ORDER BY m.wsId ASC");
		}		
	...
	*/		
	...
	public QueryResult<List<SysWsConfigVO>> findGridResult(SearchValue searchValue, PageOf pageOf) throws ServiceException, Exception {
		...
		/*
		QueryResult<List<SysWsConfigVO>> result=this.sysWsConfigDAO.findResult2(
				...
				limit);
		*/
		QueryResult<List<SysWsConfigVO>> result=this.sysWsConfigDAO.findResult3(
