...
@Controller("bsc.web.controller.PdcaManagementGridQueryAction")
@Scope
public class PdcaManagementGridQueryAction extends BaseQueryGridJsonAction {
	private static final long serialVersionUID = -9027433474995518864L;
	protected Logger logger=Logger.getLogger(PdcaManagementGridQueryAction.class);
	private IPdcaService<PdcaVO, BbPdca, String> pdcaService;
	private String message = "";
	private String success = IS_NO;
	private List<Map<String, String>> items=new ArrayList<Map<String, String>>();
	
	public PdcaManagementGridQueryAction() {
		super();
	}
	...
	private void query() throws ControllerException, AuthorityException, ServiceException, Exception {
		QueryResult<List<PdcaVO>> queryResult = this.pdcaService.findGridResult(super.getSearchValue(), super.getPageOf());
		this.success = IS_YES;
	...
	private void setGridData(List<PdcaVO> searchList) throws Exception {
		if (searchList==null || searchList.size()<1) {
			return;
		}
		this.items = super.transformSearchGridList2JsonDataMapList(
				searchList, new String[]{"oid", "title", "startDateDisplayValue", "endDateDisplayValue", "parentOid", "confirmFlag", "confirmDateDisplayValue"});
	}
	
	/**
	 * bsc.pdcaManagementGridQueryAction.action
	 */
